...
<p className = "descrip"> As everyone struggles with the 200+ communities this all shows where the communities are located and additional info, as you may know, Calgary is around the same size as New York. </p>
...
<p className = "descrip" >As the multipoligon and points to draw the limits and center on the locations of the communities were in arcGis format, we converted them with python to be google maps compatible Also we used the react library created by<a href="https://github.com/tomchentw/react-google-maps">Tom Cheng </a>to render them in my components,  that is how we implemented the <a href="https://developers.google.com/maps/documentation/javascript/tutorial" > Google Maps API </a>calls  </p>

<p className = "descrip" > Alex and I created a Postgress, database a flask server to render the data to JSON format, and as a <u>proof of concept</u> we added a Node/express/Knex server to pull the data to be consumed by asyncronous calls from the REACT front end </p>
...
The the Postgres database server currently is hosted on Amazon AWS
